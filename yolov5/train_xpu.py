...
from utils.torch_utils import (
    ...
    ModelEMA,
    de_parallel,
    # select_device,  # Not needed, we set device manually
    smart_DDP,
    smart_optimizer,
    smart_resume,
...
# Set device to XPU if available, else CPU
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    # Batch size
    if RANK == -1 and batch_size == -1:  # single-XPU only, estimate best batch size
    ...
    # No CUDA/DP mode for XPU-only

    # No SyncBatchNorm for XPU-only
    ...
    if RANK in {-1, 0}:
        ...
        if not resume:
            ...
            # No .half() for XPU-only

        callbacks.run("on_pretrain_routine_end", labels, names)

    # No DDP mode for XPU-only
    ...
    # scaler already defined for XPU above
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:
            ...
            # Backward (no GradScaler for XPU)
            loss.backward()
        ...
        # Free XPU memory after each epoch
        ...
        for f in last, best:
            if f.exists():
                try:
                    ...
                    LOGGER.warning(f"strip_optimizer failed for {f}: {e}")
                if f is best:
                    LOGGER.info(f"\nValidating {f}...")
    ...
    # No torch.cuda.empty_cache() for XPU-only
    return results
